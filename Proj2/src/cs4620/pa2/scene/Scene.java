...
public class Scene
{
	...
	/**
	 * Add a new shape to the tree with the specified material
	 */
	public void addNewShape(TreePath path, Mesh mesh, String name, Material m) throws Exception
	{
		...
		MeshNode node = new MeshNode(name, mesh, m);
	 ...
	 * Add a new shape to the tree with the specified material
	 */
	public void addNewShape(TreePath path, Class<? extends Mesh> c, Material m) throws Exception
	{
		//Get the node to insert into
		SceneNode selected = getSceneRoot();
		if (path != null && path.getLastPathComponent() instanceof TransformationNode)
		...
		MeshNode node = new MeshNode(nodeName, newObject, m);
		treeModel.insertNodeInto(node, selected, selected.getChildCount());
	}
