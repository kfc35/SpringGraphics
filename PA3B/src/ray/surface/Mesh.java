...
public class Mesh extends Surface{
	...
	private void setMeshData(float[] verts, int[] tris, float[] normals, float[] textcoords) {
		...
		for(int i=0;i<numTriangles;i++) {
			triangles[i] = new Triangle(this, tris[i*3+0],tris[i*3+1],tris[i*3+2] ,material);
		}
	...
	public void setData(String fileName) {
		System.out.println(TestFolderPath.get() + fileName);
		readMesh(this, TestFolderPath.get() + fileName);
